.diag <- "/tmp/burnP3_diag.log"
.dmem <- function(label) {
  rss_gb <- tryCatch({
    statm <- scan(paste0("/proc/", Sys.getpid(), "/statm"), quiet = TRUE)
    statm[2] * 4096 / 1e9
  }, error = function(e) NA_real_)
  cat(sprintf("[DIAG] %s | %s | RSS %.1f GB\n",
    format(Sys.time(), "%H:%M:%S"), label,
    ifelse(is.na(rss_gb), -1, rss_gb)),
    file = "/tmp/burnP3_diag.log", append = TRUE)
}
...
# Must be set before library(arrow) loads — shared.R calls library(arrow)
...
# Function to quickly identify unique partition IDs by name
...
## >> Added memory log
memRSS <- function(label) updateRunLog(sprintf("[DIAG] %s: %.1f GB RSS", label,
  as.numeric(system("grep VmRSS /proc/self/status | awk '{print $2}'", intern=TRUE)) / 1e6))

if(requiresResample) {
  progressBar(type = "message", message = "Resampling fires...")
  .dmem("enter resampling block")
  ...
  .dmem("OutputFireStatistic updated")
  ...
  .dmem("OutputFireStatistic saved")
  ...
  if(nrow(firesToReplace) > 0) {
    ...
    .dmem("DeterministicIgnitionLocation saved")
    ...
    .dmem("DeterministicBurnCondition saved")
  }
}
...
if (saveBurnMaps | saveFBPMaps | requiresResample) {
  ...
  .dmem("before updateResampledFireIDsParquet")
  ...
  .dmem("updated. Saving to SSim")

  savePartitionedParquetToSyncroSim(rawTablePath)

  .dmem("after updateResampledFireIDsParquet")
